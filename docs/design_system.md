...
- **Le tag/mention d'un camarade** utilise le symbole `@` suivi d'une liste déroulante filtrée au fur et à mesure de la frappe, le nom taggé apparaît ensuite en `--ofppt-teal` cliquable dans le texte (pas juste souligné). **Jamais `--ofppt-ink`** : c'est la couleur du corps de texte, et une mention qui la porte ne se distingue de la prose que par sa graisse — autant dire pas du tout. `--ofppt-teal` tient le seuil AA sur blanc (4,62:1), contrairement à ce que sa clarté laisse croire.
