...
- [x] **9.10 — La cloche du stagiaire s'entend** : une pastille rouge suppose qu'on regarde l'écran ; le carillon s'adresse à celui qui a le téléphone posé sur la table. Le fil est relu toutes les 45 secondes tant que l'onglet est visible, et un compte qui augmente déclenche deux notes montantes et une secousse de la cloche. Le son est **synthétisé** (`lib/son.ts`, WebAudio) et non chargé depuis un `.mp3` : deux notes tiennent en quinze lignes, ne coûtent aucun téléchargement et sonnent même quand la connexion de l'établissement flanche. Il se coupe depuis le panneau lui-même, et le réglage tient dans le navigateur.
