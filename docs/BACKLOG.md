...
- [x] **9.10 — Les deux cloches s'entendent** : une pastille rouge suppose qu'on regarde l'écran ; le carillon s'adresse à celui qui a le téléphone posé sur la table. Le fil est relu toutes les 45 secondes tant que l'onglet est visible, et un compte qui augmente déclenche deux notes montantes et une secousse de la cloche. Le son est **synthétisé** (`lib/son.ts`, WebAudio) et non chargé depuis un `.mp3` : deux notes tiennent en quinze lignes, ne coûtent aucun téléchargement et sonnent même quand la connexion de l'établissement flanche. Il se coupe depuis le panneau lui-même, et le réglage tient dans le navigateur.
  - **Le premier chargement ne sonne jamais** : arriver sur une page n'est pas un événement, et douze notifications en attente donneraient un carillon à chaque navigation.
  - **Le déverrouillage suit la règle des navigateurs** : aucun son n'est permis avant que la personne ait touché la page — une règle qui vise les publicités et qu'on ne contourne pas. Le premier appui, où qu'il soit, réveille le contexte audio ; sans cela la première notification serait muette.
  - **Pas de canal temps réel** : ouvrir une diffusion Postgres pour trois tables demanderait une publication et des policies de diffusion, beaucoup d'appareillage pour gagner trente secondes sur une notification de cours.
  - **Une seule cloche pour les deux espaces** (`components/Cloche.tsx`), après validation du carillon côté stagiaire. Trois choses les séparent — la source, les mots, et ce que compte la pastille : chez le formateur c'est un reste à faire, qui ne s'efface pas parce qu'il l'a regardé ; chez le stagiaire c'est ce qu'il n'a pas encore vu. Le reste est commun, et `PanneauNotifications` perd ses valeurs par défaut : deux jeux de mots pour le même écran auraient fini par diverger.
  - **Le carillon suit les identifiants, pas le compteur.** Chez le formateur le total descend quand il traite une tâche : répondre à une question pendant qu'un commentaire arrive laisse le compte inchangé, et un `compte > précédent` n'aurait rien sonné.
  - **Le formateur ne se fait pas sonner par lui-même** : un contrôle qu'il vient de laisser en brouillon est bien une tâche de plus, mais il le sait — il vient de l'écrire. Seuls les gestes des stagiaires carillonnent (question, commentaire, j'aime, copie rendue, devoir).
  **Test** : rendu hors ligne dans un vrai navigateur (`OfflineAudioContext`) — pic à 0,217, **zéro échantillon saturé**, 241 ms, et le profil d'énergie par tranches de 25 ms montre bien les deux notes séparées. Chemin réel éprouvé ensuite avec un vrai `AudioContext` et un vrai clic : contexte `running`, horloge qui avance, aucune exception, console vide. Animation vérifiée à l'écran : `cloche-sonne`, 800 ms, `running` puis retirée. **Carillon stagiaire confirmé à l'oreille par le porteur de projet.** **Non vérifié à l'écran** : la cloche du formateur, faute de session ouverte dans le navigateur d'essai — même code, autres réglages.

- [x] **9.11 — Le formateur n'avait jamais reçu une seule notification de commentaire** — trouvé en allant vérifier le carillon à l'écran, dans sa vraie session. Le panneau listait seize entrées : douze devoirs rendus, quatre contrôles à valider, **zéro commentaire et zéro j'aime**, alors que la base en comptait dix-sept.
  - **La cause** : les deux calculs demandaient à `profils` qui est stagiaire. Or la seule policy de lecture de `profils` ouverte à `authenticated` est `profils_select_own` — la policy permissive `profils_lecture_crochet_jeton` est réservée à `supabase_auth_admin`, c'est-à-dire au crochet de jeton. Le formateur ne lisait donc que **sa propre ligne**, l'ensemble des stagiaires revenait vide, et comme il sert de filtre (`if (!stagiaires.has(c.auteur_id)) continue`), tout passait à la trappe. En silence : ni erreur, ni ligne vide, juste un panneau qui a l'air complet. L'atome 9.2 avait réparé la pose de la question par le stagiaire ; il n'avait pas vu que la réponse n'arrivait pas.
  - **La correction** : la question se pose à `stagiaires`, table que le formateur lit partout ailleurs. Elle est d'ailleurs plus juste : est stagiaire celui qui est inscrit dans un de ses groupes, pas celui qui porte un rôle dans une table d'authentification.
  - **Un seul compteur** : `getCompteurNotifications` refaisait le calcul de son côté, avec ses propres sources — la pastille disait 7 pendant que le panneau listait 19. Le gabarit prend maintenant la longueur de la liste elle-même, et les 99 lignes du doublon disparaissent. Sans cela, la cloche qui relit toutes les 45 secondes faisait sauter le chiffre une seconde après chaque chargement de page.
  **Test** : dans la session réelle du porteur de projet, sur le serveur de développement. Avant : 16 entrées, aucun commentaire. Après : 21, dont quatre commentaires et un j'aime. Puis un commentaire de stagiaire inséré en base pendant que la page était ouverte — pastille 21 → 22, **exactement deux oscillateurs** joués sur un contexte `running` (un carillon, pas deux), une secousse de cloche relevée par observateur de mutations. Pastille identique au chargement et après relecture (20 / 20) : plus de saut. Les deux commentaires de test supprimés par identifiant, sauvegarde JSON écrite avant, comptages relevés — 19 → 17, retour à l'état initial.
  **Défaut trouvé par la même sonde** : le carillon sonnait **deux fois** au clic sur le bouton de son. L'effet était placé dans une fonction de mise à jour d'état, que React rappelle — systématiquement en développement. Sorti de là, il ne sonne plus qu'une fois : zéro oscillateur à la coupure, deux à la réactivation.

- [x] **9.12 — La participation se relit, et la distinction se fête dans le fil** (PRD §4.5).
  - **Un onglet « Participation » par séance.** La notation se fait dans une modale, un visage à la fois, puis disparaît : rien ne permettait de revenir voir ce qu'on avait donné, ni pourquoi c'est celui-là qui a été couronné. L'onglet est cette mémoire. Il **lit et ne corrige pas** — une note de participation est un jugement porté le jour même, et la retoucher une semaine plus tard déplacerait la couronne après que le groupe l'a fêtée ; le seul geste d'écriture est de rouvrir la modale. Classement par note décroissante et non alphabétique : la question qu'on se pose en ouvrant cet onglet est « qui a participé aujourd'hui », pas « où est Untel ». Une liste, pas un tableau : seize lignes de deux valeurs n'ont pas besoin de colonnes, et la barre de note s'efface sous 640 px pour laisser le chiffre.
  - **L'annonce du distingué devient une carte de fête.** Elle reste une annonce — on l'aime, on la commente, elle vieillit dans le fil — mais son texte seul ne fêtait rien. La carte reprend les signes de la modale (couronne, visage en grand, série, gerbes) dans le vocabulaire du fil : pleine largeur, pas de fond sombre, pas de bouton de fermeture. Les feux d'artifice sont **extraits** dans `components/Feux.tsx` : deux implémentations auraient divergé au premier réglage. Ils partent quand la carte entre à l'écran, une seule fois — les lancer au montage les gaspillerait sous la ligne de flottaison, les relancer à chaque passage ferait un clignotant.
  - **Reconnue par le lien, pas par le titre** : `distinctions_jour.annonce_id`, posé à la clôture. Un formateur qui renomme son annonce ne doit pas éteindre les feux d'artifice.
  - **Le formateur voit la même carte** dans ses annonces : il n'a pas écrit ce texte — la clôture l'a publié pour lui — et un paragraphe brut ne lui dirait pas ce que le groupe a reçu.
  **Test** : à l'écran, dans la session réelle, sur une séance de test créée pour l'occasion (DDOUX201, 11/09/2026). Seize notes saisies, clôture, distinction et annonce créées et liées en base ; l'onglet montre la couronne en tête, « 16 notes · moyenne 5.9 / 10 », le classement décroissant. Carte de fête vérifiée à 1512 px et **à 386 px réels** (cadre iframe, la fenêtre Chrome refusant de rétrécir) : aucun défilement en travers, aucun élément hors page dans les lignes, barres masquées, cible de 44 px tenue. **Non vérifié à l'écran** : la carte dans le fil du stagiaire — même composant, mêmes données, mais pas de session stagiaire ouverte.

- [x] **9.13 — La photo de stagiaire n'avait jamais pu être déposée** : zéro objet dans le seau, zéro fiche avec photo depuis la migration `081`. Trois causes distinctes, toutes silencieuses.
  - **Il manquait la policy de lecture** (migration `084`). Le seau est `public`, mais `public` ne concerne que le point d'accès de lecture des fichiers : la ligne de `storage.objects` reste sous RLS, et aucune policy `select` n'existait pour ce seau — les deux autres, `documents-stage` et `rendus-devoir`, en ont une. Or le dépôt utilise `upsert`, qui se traduit par `insert … on conflict do update`, et PostgreSQL exige alors de pouvoir **lire** la ligne en conflit. Il refusait avec « new row violates row-level security policy » — un message qui désigne l'écriture et envoie chercher au mauvais endroit. `remove()` était logé à la même enseigne : il lit avant de supprimer.
  - **Le sélecteur refusait les photos de téléphone.** `accept="image/png,image/jpeg,image/webp"` grise la pellicule d'un iPhone, qui est en HEIC ; et la limite de 2 Mo rejetait d'office un JPEG d'Android. On demandait à un stagiaire de convertir et redimensionner une image avant de l'envoyer. Le navigateur sait le faire : `lib/photo-navigateur.ts` décode (orientation EXIF comprise), réduit à 1024 px de côté et réencode en JPEG. Mesuré : 5,6 Mo de PNG 3000×4000 → **142 Ko**.
  - **Le retrait n'existait nulle part.** Le commentaire disait qu'il « reste sur l'écran du formateur » — mais cet écran passait lui aussi en forme compacte, où le bouton n'était pas rendu. Ajouté à la forme compacte du formateur seulement : l'en-tête du stagiaire porte déjà quatre contrôles, et on y remplace une photo en en déposant une autre.
...
- [x] **9.15 — La modération des questions de cours** (migration `085`) : des stagiaires écrivaient sous les cours des messages qui n'y avaient pas leur place, lus aussitôt par tout le groupe. Trois leviers, tenus en base — un écran se contourne en appelant l'API, une policy non.
  - **Un statut** sur chaque question et chaque réponse : ce qu'écrit un stagiaire part en attente, invisible aux autres jusqu'à validation. L'existant reste publié : vider les fils le jour du déploiement aurait semblé une panne.
  - **Deux interrupteurs** dans Paramètres → Questions des cours, pour tous les groupes du formateur : questions ouvertes ou fermées, validation obligatoire ou non (activée par défaut). Fermer ne concerne que les stagiaires — le formateur répond toujours.
  - **La suppression** : le formateur supprime tout, un stagiaire les siens — ce que la policy permettait déjà sans qu'aucun écran le propose. Confirmation obligatoire, qui dit ce qui part en cascade.
  - **La validation passe par une fonction** et non par une policy de mise à jour : le formateur rend un message visible, il ne réécrit pas ce qu'un stagiaire a signé.
  - Une porte fermée au passage : `peut_acceder_question` ne regardait que le groupe, et un stagiaire pouvait répondre à la question en attente d'un camarade dont il aurait eu l'identifiant.
  - Les notifications distinguent « attend votre validation » de « attend une réponse » ; une réponse en attente ne compte plus comme une réponse.
  **Test** : en base, rôles simulés dans des transactions annulées — attente, invisibilité pour un camarade, publication, refus de répondre à une question non publiée, refus de publier pour un stagiaire, questions fermées, validation désactivée, suppression en cascade. Puis dans la session réelle : question en attente, notification, publication, réponse de camarade en attente, publication, suppression avec sa réponse, réglages enregistrés sans écraser la charge horaire. Base revenue à son état de départ. **Non vérifié à l'écran** : la vue stagiaire.
  **À savoir** : la migration est appliquée avant le déploiement du code ; tant que ce dernier n'est pas en ligne, une question posée en production part en attente sans bouton pour la publier.

- [x] **9.16 — Le formateur répond longuement, et en Markdown** (migration `086`) : deux mille caractères et un champ d'une ligne suffisaient à un commentaire, pas à une explication.
  - **Un éditeur** pour le formateur seul : zone qui grandit, barre d'outils qui écrit la syntaxe sans la cacher, aperçu avant envoi, Ctrl ou ⌘ + Entrée pour envoyer.
  - **Vingt mille caractères** pour lui ; la contrainte de table garde ce plafond large, et `repondre_question` maintient deux mille pour un stagiaire.
  - **Le rendu suit l'auteur**, pas l'écran : le stagiaire lit la réponse mise en forme ; ses propres messages restent du texte simple. Mentions conservées. HTML jamais interprété, adresses `javascript:` neutralisées, images écartées, tableaux et blocs de code défilant dans leur cadre.
  - `AutoTextarea` accepte une référence externe, qui remplaçait la sienne et l'empêchait de grandir.
  **Test** : plafonds éprouvés en base (5 000 caractères publiés pour le formateur, 2 500 refusés pour un stagiaire, 20 001 refusés par la table). Dans la session réelle, sur DES101 — groupe sans stagiaire, pour n'exposer personne : outils sur sélection, aperçu complet (titre, tableau, code, citation, tâches, liens), essais d'injection affichés en texte sans exécution, envoi et rendu dans le fil. À 390 px : ni page ni carte ne défilent en travers, outils et envoi à 44 px. Question d'essai effacée, sauvegarde écrite avant.

- [x] **9.17 — Le recalcul du planning effaçait les séances passées non cochées** : un créneau ajouté puis retiré dans l'emploi du temps, et la séance du mardi 15/09 8 h 30 de DDOUX201 avait disparu.
  - **La cause** : `replanifier` détachait toutes les séances « à faire » datées, passées comprises, et repartait d'aujourd'hui. Le formateur coche souvent le lendemain : la séance de la veille, pas encore cochée, était renvoyée après aujourd'hui, et son créneau — passé — ne pouvait plus rien recevoir. Tout le planning glissait d'un créneau.
  - **Le passé est figé** : une séance dont l'heure est passée reste en place, cochée ou non. L'heure est lue à Casablanca, où que tourne le serveur — sur un serveur en UTC, `maintenant()` décalait la nuit d'une heure.
  - **Aucun créneau déjà occupé n'est rempli** : sans cette règle, laisser une séance passée en place aurait créé deux séances à la même heure. La génération lancée à la main garde sa liberté de partir d'une date passée.
  - **Une séance préparée n'est plus supprimée par une fusion** : elle porte la séance où elle tombe ; si deux séances préparées devaient fusionner, le recalcul refuse avant la moindre écriture. Et si la génération échoue, les séances détachées retrouvent leur date au lieu de rester hors du calendrier.
  - **Second défaut, trouvé en comparant avant et après** : chaque recalcul ré-accrochait la nature à l'intitulé — « — theorique — theorique — theorique… », jusqu'à 853 caractères. L'intitulé est désormais composé sans répétition, et les 160 intitulés en base ont été nettoyés (sauvegarde écrite avant).
  **Test** : 17 règles éprouvées une à une, puis 12 cas de composition d'intitulé dont la normalisation des 160 intitulés réels. En conditions réelles sur DES101 — groupe sans stagiaire, planning sauvegardé : créneau ajouté puis retiré, la séance passée du 11/09 reste en place, et le planning revient **à l'identique** (32 séances, dates, heures, durées, contenus). Puis DDOUX201 restauré à partir du 15/09 : mardi 15/09 8 h 30 de retour avec B.2 théorique, 96 séances, 480 h, aucune sans date, aucun créneau en double. Le serveur de développement ayant redémarré au milieu de la restauration, la seconde passe a achevé le travail sans doublon — la règle des créneaux occupés a tenu.
  **Reste** : les intitulés de DDOUX201 annoncent encore, pour 68 séances, des objectifs qu'elles ne couvrent pas toutes — des restes de fusions passées, mêlés à des fusions légitimes (48 parties de la répartition font 2 h 30 ou 7 h 30). Les démêler suppose de rejouer la répartition contre les séances ; ces intitulés nourrissent les générations par IA.

- [x] **9.18 — L'alternance des créneaux** (migration `087`) : un groupe qu'on accélère alterne 25 h et 27 h 30 — un créneau de 2 h 30 qui ne revient qu'une semaine sur deux. D'autres ne reviennent qu'une fois par mois. Déclarés hebdomadaires, ils plaçaient deux fois trop de séances ; non déclarés, aucune.
  - **Trois rythmes par créneau** : chaque semaine ; une semaine sur deux, à partir d'une première date qui fixe ses semaines (dans les deux sens) ; une fois par mois, au même rang du jour que la première date — déclaré un quatrième mercredi, il revient chaque quatrième mercredi. La base refuse un rythme alterné sans première date, ou dont la première date ne tombe pas le bon jour.
  - **Le recalcul en tient compte** comme d'habitude : un créneau n'ouvre que les jours où il a lieu ; jours fériés et vacances sautés de la même façon.
  - **À l'écran** : « Répétition » et « Première fois le » à l'ajout comme à la modification — la date part du prochain jour choisi et suit le jour s'il change. La grille et la liste mobile portent « 1 sem. sur 2 » ou « 1× par mois », le PDF aussi. Le résumé passe du nombre de créneaux aux heures : « 25 h ou 27 h 30 selon la semaine », calculé sur un an de semaines réelles.
  **Test** : 17 règles éprouvées une à une (parité des semaines dans les deux sens et à travers l'heure d'hiver, rang mensuel, 25 h / 27,5 h). En conditions réelles sur DES101, planning sauvegardé : créneau mercredi une semaine sur deux à partir du 23/09 → 23/09, 07/10, 21/10, 04/11, vendredis inchangés, séance passée du 11/09 en place ; modifié en mensuel → 23/09, 28/10, 25/11, 23/12, 24/02 (le 27/01 tombe dans les vacances) ; retiré → planning **identique** à la sauvegarde, intitulés compris. Formulaire à 390 px sans débordement. **Non vérifié à l'écran** : le PDF.
...
- [x] **10.1 — Le sujet imprimé reprend la présentation adoptée** · branche `controle-sujet-pdf`
  ...
  **Fait** : un seul générateur pour trois pièces — le sujet distribué (cadre nom / CEF / CNE / note, « Nom et CEF » au pied des pages suivantes), le sujet à viser (cachet sur chaque page) et le corrigé (bonnes propositions cochées, réponse attendue à la place de la composition). L'ancien gabarit encadré est supprimé. L'espace de réponse suit la question : lignes proportionnées au barème pour une question ouverte (1 pt → 5 lignes, 4 pts → 12), cadre sans lignes pour un exercice (4 pts → 80 mm), rien pour un QCM ; une question trop haute pour ce qui reste part entière sur la page suivante, et c'est sa place de composition qui se resserre si elle ne tient pas même sur une page vierge. L'en-tête d'une épreuve se construit désormais pour un brouillon non enregistré : on imprime la version qu'on a sous les yeux, et le fichier se nomme `Sujet_CC1_DDOUX201_M202_…` / `Corrige_…`. La durée figure dans le cartouche même sans séance à l'emploi du temps.
...
- [x] **10.3 — Brouillons et versions** · branche `controle-brouillons`
...
- [x] **10.3c — Choisir un contrôle, et le voir comme un stagiaire** · branche `controle-apercu`
  Demande du 17/09 : la liste déroulante « Nouveau contrôle / contrôle existant » ne dit rien et ouvrir un brouillon ressemblait à créer un contrôle ; un onglet Aperçu entre Éditeur et Copies.
  **Test** : ouvrir un brouillon existant, voir ses questions ; passer à l'Aperçu ; revenir à « Nouveau contrôle » puis au brouillon.
  **Fait** : cartes des contrôles du groupe sur le module (nature, statut, titre, nombre de questions et points, date) et carte « Nouveau contrôle », à la place de la liste déroulante ; badge d'état dans l'en-tête ; un contrôle qui a des questions s'ouvre à l'étape Questions ; onglets Éditeur · Aperçu · Copies — l'Aperçu rend la passation du stagiaire sur l'état affiché, modifications non enregistrées comprises, sans corrigé, remise désactivée, rien gardé dans le navigateur.
  **Vérifié** : sur le CC M202 du formateur (non modifié) : ouverture à l'étape 3 avec ses 9 questions, Aperçu à 9 questions et remise désactivée, aller-retour Nouveau contrôle → brouillon sans « modifications ».

- [x] **10.4 — Les contrôles de test, sur une ou plusieurs séances** · branche `controle-test`
  ...
  **Fait** : type `TEST` (migration 090), barème libre `bareme_total` (20 par défaut), périmètre `seance_ids` enregistré pour tout contrôle et recoché à la réouverture. Quatrième nature « Contrôle de test » à l'étape 2, avec son barème ; l'étape 1 propose toutes les séances du module (faites et à venir) et ne coche que la dernière faite ; changer de nature vide le périmètre. Génération : total libre, séances choisies obligatoires. Code d'épreuve TEST1, nature « Contrôle de test ». Écarté des échéances, du calendrier, de l'emploi du temps et de la moyenne du stagiaire ; page module : « 1 contrôle · 1 test » et colonne Nature. En attendant 10.5, un test validé reste invisible au stagiaire (politique, sujet et route de remise).
  **Vérifié** : test M202 sur 08/09 et 10/09, barème 10 → génération à 10 points sur ces deux séances, enregistré (type TEST, bareme_total 10, 2 séances), rechargé sans « modifications » et séances recochées. Stagiaire simulé, test passé validé dans une transaction annulée : 0 ligne, 0 question — témoin CC validé visible avec ses 9 questions. Calendrier : le test n'apparaît pas. Test sauvegardé puis supprimé (6/17/7 → 5/9/6). Non vérifié : le PDF d'un test (code TEST1).

- [x] **10.5 — La passation d'un contrôle de test** · branche `controle-test-passation`
  ...
  **Fait** : `ouvert_le` / `ferme_le` (migration 091) — un test chronométré est un test ouvert dont la fermeture est fixée d'avance, « ouvert » se lit à l'heure (fonction `controle_ouvert`), sans tâche planifiée. Panneau « Passation en ligne » : ouvrir au groupe pendant 15 min à 2 h ou sans limite, fermer maintenant (confirmation), rouvrir ; test validé et enregistré exigé ; un test ouvert refuse d'être modifié ou repassé en brouillon. Stagiaire : le test apparaît dans ses contrôles et sa cloche dès l'ouverture, se compose seulement pendant l'ouverture (sujet et remise refusés sinon, une minute de grâce), compte à rebours tiré de `ferme_le` et remise automatique à zéro, page « test fermé » ; note visible seulement après publication ; hors moyenne.
  **Vérifié** : test d'essai M202 validé. Stagiaire simulé : jamais ouvert → invisible ; ouvert 15 min depuis le panneau (14 h 44 → 14 h 59) → visible, 2 questions, aucun drapeau « correcte » ; modification refusée pendant l'ouverture ; copie simulée → remise connue, note invisible, puis visible une fois publiée (transaction annulée) ; copie listée 3/4 dans Copies ; fermé depuis le panneau → visible, sujet refusé ; chrono échu → sujet refusé, chrono en cours → 2 questions ; ouverture impossible sur un CC (contrainte). Compte à rebours sur page temporaire : remise automatique à zéro avec la réponse écrite (refusée 403, compte formateur) — défaut trouvé et corrigé : la remise n'était pas réarmée quand la fin changeait. Essai sauvegardé puis supprimé (6/11/1 → 5/9/0). Non vérifié avec un vrai compte stagiaire : la remise acceptée par la route pendant l'ouverture et refusée après.

- [x] **10.6 — L'analyse de compréhension** · branche `controle-analyse`
  ...
  **Fait** : table `analyses_controle` (migration 092, formateur seulement), chaque analyse conservée et relisible. Chiffres calculés sur les copies corrigées (`lib/analyse-comprehension.ts`) : taux et réussite par question, réponses vides, moyenne, médiane, répartition, stagiaires sous 50 %. Modèle : réponses pseudonymisées (« Stagiaire A » dans l'ordre des notes), noms et prénoms du groupe effacés aussi des réponses et des commentaires de correction, notions des séances évaluées (`seance_ids`), séances à venir ; il rend niveau, erreurs fréquentes par question, état des notions, accompagnement des stagiaires sous 50 %, ajustements datés sur une séance à venir. Validation côté serveur : états bornés, pseudonymes existants seulement, dates prises dans les séances à venir. Onglet « Analyse » : niveau et chiffres, barre de répartition, question par question, notions, stagiaires (nom + pseudonyme), ajustements, historique des analyses, alerte si de nouvelles copies ont été corrigées depuis.
  **Vérifié** : jeu de 8 copies d'essai sur M202 aux erreurs plantées — Q1 confusion persona / utilisateur réel (6/8), Q2 critères démographiques au lieu de comportementaux, Q3 réussie, Q4 laissée vide (5/8). Les quatre retrouvées mot pour mot ; les 5 stagiaires attendus signalés ; 4 notions du référentiel reconnues. Aucun nom dans le texte envoyé (reconstruit : « Nadia, tu cites… » part en « Stagiaire F, tu cites… ») ni dans les lectures. Deux défauts trouvés et corrigés : notions et stagiaires vidés par une validation trop stricte (pseudonyme abrégé) ; un constat recopiait des numéros de questions faux — le modèle ne les répète plus. Essai sauvegardé puis supprimé (6/13/8/3 → 5/9/0/0).

- [x] **10.7 — Le compte de test du formateur** · branche `stagiaire-de-test`
  Demande du 17/09 : un compte stagiaire de DDOUX201 pour passer un contrôle soi-même avant de le lancer, absent de la liste des stagiaires.
  **Test** : créer le compte ; il n'apparaît dans aucune liste ; il voit un contrôle en brouillon qu'un vrai stagiaire ne voit pas ; le compte formateur reste formateur.
  **Fait** : `stagiaires.est_test` (migration 093, un par groupe) ; section « Compte de test » en bas de la page du groupe — créer (adresse proposée : celle du formateur avec « +groupe »), renvoyer le lien, supprimer ; le mot de passe est choisi par le formateur via le lien reçu, jamais vu ni saisi par l'outil. Garde-fou : adresse du formateur ou d'un compte non stagiaire refusée (sans lui, l'invitation aurait fait d'un compte formateur un compte stagiaire). Le compte voit les contrôles du groupe en brouillon et les tests non ouverts (politique, sujet, remise en base et route), bandeau « Compte de test » dans l'espace stagiaire, « Effacer ma copie de test et recommencer ». Exclu de la liste et du compteur de stagiaires, de l'appel, de la participation et de la distinction, des mentions, des envois d'annonces et d'invitations groupées, des stages, des devoirs, du tableau de bord, de l'émargement et du dossier d'épreuve, de l'analyse ; sa copie est marquée « Compte de test » dans Copies et ne bloque pas la modification d'un contrôle (elle est effacée).
  **Ajout du 17/09 — chronomètre du compte de test** (branche `chrono-compte-test`) : un brouillon ou un test sans heure de fermeture n'affichait aucun minuteur. Le compte de test a désormais le sien, sur la durée du contrôle, à partir de sa première ouverture (gardée sur l'appareil, remis à zéro en recommençant), avec la même remise automatique. Vérifié sur page temporaire (durée 0,01 h) : décompte affiché « durée du contrôle », reprise à 0:13 après rechargement, remise automatique à zéro avec la réponse écrite.
  **Vérifié** : adresse formateur refusée avant toute écriture ; compte créé pour DDOUX201 (rôle stagiaire, lien envoyé à l'adresse +ddoux201) ; formateur toujours « formateur » ; simulé : le compte de test voit le brouillon « Test d'entraînement » et ses 9 questions, un vrai stagiaire du groupe n'en voit aucun ; page du groupe toujours à 15 stagiaires. Non vérifié : la connexion elle-même, qui passe par le choix du mot de passe du formateur.

- [x] **10.8 — La remise confirmée, et la copie corrigée à relire** · branche `copie-corrigee`
  Demande du 17/09 : à la remise, une fenêtre de confirmation, la saisie grisée et le chronomètre arrêté ; après publication, le stagiaire revient sur le contrôle et voit, question par question, sa réponse, la bonne réponse et le commentaire du formateur.
  **Test** : rendre une copie ; lire une copie publiée ; vérifier que la bonne réponse ne sort ni avant publication ni pour un autre stagiaire.
  **Fait** : passation — à l'acceptation par le serveur, fenêtre « Votre copie est bien rendue » (heure de remise, ce qui vient ensuite), réponses verrouillées et grisées, chronomètre figé « arrêté », bouton « Copie rendue », bandeau « Copie rendue à … » ; la remise automatique passe par la même fenêtre. Copie : avant publication, « Votre copie est bien arrivée » ; après, présentation de la passation avec la note dans la couverture et, par question, points colorés, énoncé et données, pour un QCM chaque proposition avec la case cochée et « Juste » / « Fausse », sinon « Votre réponse » à côté de « Réponse attendue », et « Commentaire du formateur ». Fonction `get_ma_correction` (migration 094) : les propositions justes et la réponse attendue ne sortent qu'à l'auteur d'une copie publiée.
  **Vérifié** : en base, copie d'essai du compte de test — non publiée : 0 ligne ; publiée : les propositions avec « correcte » ; un autre stagiaire du groupe, même publiée : 0 ligne (essai supprimé, 6/10/1 → 5/9/0). À l'écran sur page temporaire : copie corrigée (QCM, question ouverte, exercice avec données) ; remise simulée acceptée → fenêtre ouverte, chrono figé à 29:54, champs désactivés, bouton « Copie rendue ».

- [x] **10.9 — Correctif : la publication d'un résultat ne publiait rien** · branche `publication-resultat`
  Signalé le 17/09 : après correction de la copie du compte de test, « Publier le résultat » n'affiche rien côté stagiaire.
  **Cause** : depuis la migration 077, la publication écrivait `publie_le` directement, alors qu'aucune politique n'ouvre la mise à jour de `passations_controle` au formateur ; la base ignorait l'écriture sans erreur et l'écran annonçait le succès. Aucun résultat n'a jamais été publié (0 en base). Trouvé au passage : la correction plafonnait la note à 20, faux pour un EFM (40) ou un test à barème libre (le test d'entraînement est sur 30).
  **Fait** : fonction `publier_resultat` (migration 095) — vérifie l'accès au contrôle, refuse une copie non notée, écrit et rend la date ; l'action lève une erreur si rien n'est enregistré. Plafond retiré : `corriger_passation` borne déjà la note au total réel.
  **Vérifié** : transaction annulée sur la copie du compte de test — publiée par le formateur, le compte de test voit sa copie et les 9 questions corrigées ; un vrai stagiaire est refusé (« Vous ne publiez pas les résultats de ce contrôle ») ; la copie reste non publiée en base, à publier par le formateur.
...
- [x] **P.2 — Aucun contrôle n'est lisible avant d'être ouvert** · branche `ouverture-controles`
  Signalé le 24/09 : un contrôle validé s'affichait aussitôt aux stagiaires, qui pouvaient lire le sujet avant l'épreuve ; le panneau d'ouverture n'existait que pour les contrôles de test ; et le menu « Pendant 30 min » faisait saisir une durée que le contrôle porte déjà.
...
- [x] **11.2 — Le chapitre, et la progression du stagiaire** · branche `cours-chapitre`
  ...
  **Fait** : table `progression_chapitre` (migration 097), ouverte au seul stagiaire concerné — ni ses camarades ni le formateur n'y accèdent. Chapitre : sommaire du module à gauche (replié sur téléphone) avec la partie, l'état de chaque chapitre et celui qu'on lit ; « Chapitre N sur M » en tête ; en pied, « J'ai terminé ce chapitre » (réversible), « Précédent » et « Chapitre suivant : … ». Sommaire du module : barre de progression, « Reprendre au chapitre N » (ou « Commencer », ou « Revoir » quand tout est terminé), coche verte sur les chapitres terminés. Liste des modules : barre et « 3 / 8 chapitres ».
  **Vérifié** : en base, la ligne de progression du compte de test est vue par lui seul — 0 ligne pour un autre stagiaire, 0 pour le formateur —, et un stagiaire ne peut pas écrire celle d'un autre (refus de la politique). Écrans relus sur page d'aperçu temporaire (supprimée) : sommaire avec progression 38 %, coches, reprise au chapitre 4, et sommaire latéral avec le chapitre courant en évidence.

- [x] **11.3 — Le quiz d'auto-évaluation d'un chapitre** · branche `quiz-chapitre`
  ...
  **Fait** : table `quiz_chapitre` (migration 098) — un quiz par chapitre, écrit une fois puis servi tel quel, lisible des stagiaires du groupe et du formateur, jamais écrit par un stagiaire (la route insère avec la clé de service après vérification de l'accès). Route `/api/generate/quiz` : le support est donné au modèle du formateur propriétaire — comme la correction d'une copie —, qui rend 5 QCM à une seule bonne réponse avec explication ; validation côté serveur (3 à 4 propositions distinctes, index de la bonne réponse valide), refus si moins de 3 questions tiennent. Écran : une question à la fois, correction dès la réponse, explication affichée même quand c'est juste, score final avec un mot selon le résultat, « Recommencer ». Rien n'est enregistré : ni tentative, ni score, ni remontée au formateur.
  **Vérifié** : quiz écrit en 5 s sur « Mettre en forme les données recueillies » (M202) — les 5 questions portent sur le cours (hiérarchisation, matrice importance × fréquence, types convergents), l'explication cite le cours (« On la garde, mais on ne décide rien avec elle seule »). À l'écran : réponse fausse → bonne réponse en vert, réponse donnée en rouge, explication dessous. Deuxième appel servi depuis la base, sans régénération.

- [x] **11.4 — Le quiz de partie, toutes les trois séances** · branche `quiz-partie`
  ...
  **Fait** : table `quiz_bilan` (migration 099), une ligne par module, groupe et rang — deux groupes n'ont pas les mêmes supports. Les jalons se calculent sur les chapitres du module, trois par trois ; un groupe incomplet n'en donne pas (un « bilan » sur deux cours ne ferait pas le lien qu'on en attend). Route `/api/generate/quiz/bilan` : le jalon est relu sous l'identité de l'appelant, les trois supports se partagent le budget du prompt, et le modèle du formateur propriétaire rend 8 QCM couvrant les trois chapitres dont au moins deux questions qui les relient ; le bilan est réécrit si un support a changé. Le lecteur de quiz est devenu un composant partagé (`components/QuizJoueur.tsx`), le quiz de chapitre s'en sert aussi. Écran : une page par bilan avec les trois chapitres en tête, une carte jalon dans le sommaire après le troisième chapitre, et un renvoi en fin de ce chapitre.
  **Vérifié** : sur M202 (DDOUX201, 13 chapitres), le sommaire affiche 4 jalons — le 13ᵉ chapitre, seul de son groupe, n'en a pas. Bilan 1 écrit en 9 s : 8 questions réparties 3 / 2 / 3 sur les chapitres 1 à 3, deux d'entre elles relient deux chapitres ; réponse juste → explication citant le cours. Deuxième ouverture servie depuis la base, sans appel au modèle. Le chapitre 3 porte le renvoi au bilan, le chapitre 2 non. Rang inexistant → page introuvable. Le quiz de chapitre fonctionne toujours après l'extraction du lecteur.

- [x] **11.5 — L'espace stagiaire au même niveau** · branche `espace-stagiaire-v2`
  ...
  **Fait** : un en-tête commun (`app/espace-stagiaire/EnTete.tsx`) — surtitre, titre, ligne de résumé — repris par les cinq écrans ; les devoirs n'en avaient aucun et s'ouvraient directement sur une carte. Les devoirs se rangent en deux listes sous leur intertitre, « À rendre » et « Remis », comme les parties d'un module, avec le nombre de devoirs en retard dans l'en-tête ; la carte d'un devoir suit la grammaire commune (pastille d'état, titre, lignes d'appoint en mono, étiquette à droite) et son corps s'aligne sur le titre sur grand écran seulement — une zone de saisie retranchée de 49 px ne tiendrait plus sur un téléphone. L'emploi du temps ne montre plus l'intitulé brut d'une séance : `lib/libelle-seance.ts` le redécoupe en éléments, le premier fait le titre, les autres se lisent dessous, et les natures empilées par d'anciens recalculs disparaissent. Dans la barre haute, « Emploi du temps » ne revient plus à la ligne : la version courte tient tant que le nom du stagiaire et les cinq onglets ne rentrent pas ensemble.
  **Élargi ensuite** : l'espace tenait dans 896 px et laissait la moitié d'un écran vide. Le gabarit monte à 1152 px, puis 1440 px au-delà de 1280 px de large. Le chapitre en profite pleinement — sommaire à gauche, document à droite —, ses tableaux et ses encadrés en grille trouvent enfin leur place, et le texte suivi garde une colonne de 78 caractères (le texte d'un encadré, lui, remplit sa boîte). Les écrans de liste gardent une colonne : 1024 px pour les devoirs, les contrôles, l'emploi du temps, les modules et le sommaire, 768 px pour le fil et le bilan — au-delà, la note d'un contrôle se retrouve à dix centimètres de son titre. Au passage, le chapitre ne mettait pas son sommaire replié au-dessus du cours sur téléphone mais à côté, chacun sur une demi-largeur (depuis la 11.2) : il s'empile désormais.
  **Vérifié** : les cinq écrans ouverts en 375 px puis en 1024 px — même en-tête, même carte, aucun débordement ni retour à la ligne dans la navigation. Emploi du temps : « C.2 — Trouver des solutions techniques — pratique · D.1 — Créer des personas — theorique · D.1 — … — pratique · D.2 — … » devient « Trouver des solutions techniques » suivi de deux éléments, le doublon de D.1 fondu en une ligne. Devoirs : une remise faite depuis le compte de test fait passer le devoir dans « Remis » avec sa pastille verte et la date de remise, et le compteur de l'en-tête suit ; les deux lignes d'essai ont été sauvegardées puis supprimées par identifiant (115 → 113 rendus).
...
## Phase 12 — Le classement d'une épreuve, annoncé au groupe (demande du 25 septembre 2026)

- [x] **12.1 — Le podium d'un contrôle, publié dans le fil** · branche `classement-controle`
  Quand le formateur a fini de publier les copies d'un contrôle, un bouton annonce le classement au groupe : les trois premiers sur un podium, puis la classe rang par rang, chacun retrouvant sa ligne. Un classement par épreuve — CC 1, CC 2, EFM —, jamais une moyenne cumulée.
  **Test** : sur un contrôle dont les résultats sont publiés, le podium s'affiche côté formateur ; l'annonce publiée porte le podium dans le fil du groupe, et chaque stagiaire y voit sa ligne mise en avant.
  **Fait** : table `classements_controle` (migration 100), rattachée à l'annonce comme `distinctions_jour` — c'est le lien qui fait la carte, pas le titre. Le classement est **figé** à l'annonce : une copie recorrigée ensuite ne doit pas réécrire en silence un podium déjà lu. Ne sont classées que les copies **publiées** — une note que son stagiaire n'a pas vue ne peut pas s'afficher devant le groupe — et jamais celle du compte de test. Les ex æquo partagent leur rang, et le suivant saute d'autant. Le podium (`components/PodiumClassement.tsx`) sert des deux côtés : aperçu chez le formateur, carte dans le fil. Réannoncer réécrit l'annonce existante, avec ses j'aime et ses commentaires, au lieu d'en empiler une seconde.
  **Feux d'artifice continus** : le composant `Feux` ne jouait que trois gerbes ; une annonce se relit le soir ou la semaine d'après, et une fête jouée une fois ne dit plus rien à qui arrive après. En mode `continu`, une gerbe part toutes les soixante-dix images, les particules éteintes sortent du tableau, et l'animation s'arrête dès que le podium quitte l'écran — comme elle ne démarre pas si le système demande à réduire les animations.
  **Médailles plutôt que marches** (demande du 25/09/2026) : le podium en trois colonnes ne savait pas loger deux stagiaires sur la même marche. Les médailles se donnent désormais à la **note** et non à la place — trois stagiaires à 16/20 sont tous premiers, sous la même couronne d'or —, et la forme devient une liste : trois blocs or, argent, bronze, chacun pouvant tenir plusieurs noms avec photo et note. L'argent est le gris neutre de la palette : à côté de l'or et du bronze, une couronne sarcelle ne se lit pas comme une médaille.
  **Vérifié** : sur le test du 17/09 (15 copies, 14 hors compte de test), médailles ABDELALI ARMAS 16,25 / KHADIJA FAHMI 16 / SARA NAIT AISSI 15,5, moyenne 12,5/20, ex æquo à 12/20 classés 7ᵉ et 7ᵉ puis 9ᵉ dans la suite — et le compte de test absent. Regroupement par note vérifié sur trois cas : trois ex æquo en tête donnent bien une 1ʳᵉ place à trois noms, puis une 2ᵉ et une 3ᵉ places aux notes suivantes. Sur le CC 1, dont aucune copie n'est publiée, aucun bloc de classement n'apparaît. Politiques vérifiées dans une transaction annulée : un stagiaire du groupe lit le classement, un compte étranger ne lit rien.
...
- [x] **13.3 — La lecture du niveau par l'IA** · branche `suivi-stagiaire`
  ...
  **Fait** : table `lectures_suivi` (migration 102), une lecture par stagiaire, lisible et écrivable du seul formateur de son groupe — jamais montrée au stagiaire. Route `/api/generate/suivi` : le suivi est relu sous l'identité de l'appelant, la lecture conservée est servie telle quelle, et `regenerer` la relance. Le modèle reçoit des chiffres et des énoncés de questions, **ni nom ni prénom** — comme l'analyse de compréhension (§4.7bis). Il rend quatre champs : un résumé, ce qui est acquis, ce qui manque, ce qu'il faut reprendre ; une liste vide ne s'affiche pas, et la consigne lui interdit de conclure sur un niveau que les données ne montrent pas. L'écran ouvre sur la dernière lecture écrite, sans appeler le modèle.
  **Vérifié** : sur un stagiaire sans aucun quiz, le modèle refuse de se prononcer — « En l'absence de quiz, on ne peut pas se prononcer sur son niveau de maîtrise des notions » — et ne retient comme acquis que des faits (8 devoirs sur 8, 17,5/20 au CC 1). Sur le compte de test, qui a passé un quiz à 0/5, les quatre lacunes citent les **vraies** questions ratées (segment d'utilisateurs, lecture d'un jugement fiabilité/pertinence, apport du qualitatif, biais d'une question d'entretien) et aucun bloc « acquis » n'est inventé. Lecture écrite en 15 s, conservée, rouverte sans nouvel appel.
