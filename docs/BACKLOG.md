...
- [ ] **2b.3** — Prompt : *"Ajoute à `seances` la colonne `suggestion_pedagogique_id` (obligatoire — c'est l'objectif d'apprentissage de la séance) et `nature` (théorique/pratique). Toute séance porte un objectif pédagogique : c'est lui qui fonde la fiche de préparation et le support."*
